...
class ArchetypalAnalysis:
    ...
    def __init__(self,
        ...
        self.adata.raw = None
        self.unnorm_umap = np.array(unnorm_umap)
        self.norm_umap = None
    ...
    def compute_archetypes(self, pc_ratio=.05, r=20, display=False, use_old_umap=True):
        """
        ...
        Parameters
        ----------
        pc_ratio ：float (default=10)
            Ratio of gene dimension to represent `conditional_number` for ID estimation (Fisher Separability test)
            Fisher Separability test select the top PCs with eigenvalues <= max(Eig) / conditional_number
...
`       r : int
        ...
        arche_dict : dict
            Hierarchical strucuture major_archetype -> its fine-grained neighbor archetypes
    ...
    def check_nonnegative_integers(self,X: Union[np.ndarray, sparse.spmatrix]):
    ...
    def find_markers(self, n_markers=30, display=False):
        ...
        for col in self.arche_df.columns:
            ...
            #print(adata.X)
            if self.check_nonnegative_integers(adata.X):
    ...
    def _save_fig(self, fig, lgds, default_name):
        ...
        fig.savefig(os.path.join(self.outdir, filename+'.eps'), bbox_extra_artists=lgds, bbox_inches='tight', format='eps')

    def plot_archetypes(self, major=True, do_3d=False, lgd_ncol=1, norm_umap=True, figsize=(6, 4)):
        """
        ...
        if do_3d:
            ...
            ax.scatter(U[:self.n_spots, 0], U[:self.n_spots, 1], s=0.5, c='lightgray')
            for i, label in enumerate(self.arche_df.columns):
                lbl = int(label.split('_')[-1])
                if lbl in arche_indices:
                    idxs = self.arche_df[label]
            ...
            # Color background & archetypal spots
            ax.scatter(self.unnorm_umap[:self.n_spots, 0], self.unnorm_umap[:self.n_spots, 1], s=0.5, c='lightgray')
            for i, label in enumerate(self.arche_df.columns):
                lbl = int(label.split('_')[-1])
                if lbl in arche_indices:
                    idxs = self.arche_df[label]
                    ax.scatter(self.unnorm_umap[idxs, 0], self.unnorm_umap[idxs, 1], marker='o', s=3, color=colors[i], label=label)

            # Highlight archetype
            #idxs = self.n_spots + arche_indices
            #ax.scatter(self.unnorm_umap[idxs, 0], self.unnorm_umap[idxs, 1], s=5, c='r', marker='^')
            #for j, z in zip(arche_indices, self.unnorm_umap[idxs]):
               # ax.text(z[0], z[1], str(j))
            lgd = ax.legend(loc='right', bbox_to_anchor=(2, 0.5), ncol=lgd_ncol)
            ax.grid(False)
            ax.axis('off')
